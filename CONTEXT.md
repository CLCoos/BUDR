...
**Sidst opdateret (manuelt):** 2026-04-24 — **Beboernavne (fornavn + initial):** ny migration `20260424112000_resident_name_display_mode_and_name_fields.sql` tilføjer `care_residents.first_name`, `care_residents.last_name` og `organisations.resident_name_display_mode` (`first_name_initial|full_name|initials_only`). Ny helper `src/lib/residents/formatName.ts` + hook `src/lib/residents/useNameDisplay.ts` med org-mode via `CurrentOrgProvider` i `PortalShell` (`src/contexts/CurrentOrgContext.tsx`). Ny settings-API `PATCH/GET /api/portal/resident-name-display-mode` og UI i `care-portal-dashboard/settings` for at skifte visning. Navnevisning opdateret i centrale flader: dashboard beboerliste, alerts, medicin, handover, resident-360 og care-portal-residents; avatarer bevarer initialer, mens tekstfelter bruger formatteret navn. 360°-header bruger fuldt navn. **Care Portal — global afdeling:** `CarePortalDepartmentProvider` i `PortalShell` + demo-skallen (`CarePortalDemoShell`); **manuel** valg gemmes som `budr-care-portal-department-mode=fixed` + `budr-care-portal-department` (inkl. `alle`). Uden fixed: **auto** fra (1) Supabase Auth `user_metadata.work_house` / `default_care_house` / `care_house` / `afdeling` (`parseStaffWorkHouseMetadata`), (2) ellers demo-/pilot-vagt (`loadShifts` + `inferDepartmentFromDemoShifts` og `VAGTPLAN_CORE_SHIFT_LOCATIONS` i `vagtplanInferDepartment.ts`); opdatering ca. hvert minut + ved `storage` / `budr-demo-shifts-updated` efter `saveShifts`. Én `DepartmentSelect` i topnav (live + demo) og mobil-header; widgets afgrænses via `resolveStaffOrgResidentsForDepartment` / hus fra `onboarding_data`. **Dokumentsøgning:** ved aktiv afdeling + søgning vises først **På [valgt afdeling]**, derefter **Alle afdelinger**; uden søgning opfører søgefeltet sig som før. Delt logik: `src/lib/carePortalHouse.ts` (`onboardingHouseToCareHouse`). **Care Portal dashboard:** Nøgletal + zone-2-widgets bruger `getWidgetStatus` / `--status-*` på top-border (2px); per-widget hus-dropdowns er fjernet til fordel for den globale vælger; hurtig journal hedder **Ny journal**, flydende FAB fjernet — primær knap i header + `[+]` på beboer-rækker (hover/touch). **Care Portal layout:** `OverrapportPanel` rendes med `createPortal` til `document.body` (så `fixed` altid er viewport), og `cp-page-enter` animerer kun opacity (ingen transform på wrapper). Tidligere kunne `fixed` forankres til `main` via transform/forældre og give vandret scroll + mast dashboard. **Sentry incident-alerting:** `@sentry/nextjs` installeret. Manuel wizard-ækvivalent opsætning (wizard var interaktiv): `instrumentation.ts`, `instrumentation-client.ts`, `sentry.server.config.ts`, `sentry.edge.config.ts` + `withSentryConfig` i `next.config.mjs` (inkl. log-treeshake). `netlify.toml` indeholder nu `SENTRY_DSN` key under `build.environment` (værdi sættes i Netlify UI). Midlertidig smoke-test route er fjernet igen efter verifikation i produktion. **Offboarding v1 i intern admin (`/budr-admin`):** ny migration `20260422182000_organisations_deactivated_at.sql` tilføjer `organisations.deactivated_at`; knap **Deaktivér organisation** på admin-oversigten kalder `POST /budr-admin/deactivate` (service role) og sætter timestamp; ny **Eksportér data (ZIP)** via `GET /budr-admin/export/[orgId]` genererer ZIP med CSV-filer for org-scopede tabeller (service role). Middleware blokerer herefter staff-login for deaktiverede orgs og sender brugeren til login med beskeden **“Denne konto er deaktiveret — kontakt BUDR.”**. **Intern admin (`/budr-admin`) udvidet med tværgående sundhedstjek + superadmin-dashboard-grafer:** service-role overblik viser nu tabel med navn, oprettelsesdato, antal beboere, antal staff, seneste `audit_logs`-aktivitet, risikobadges, filter (`all/critical/warning/healthy`) med URL-param (`?health=`) samt tællere. Derudover vises pr. org en 30-dages linjegraf for daglig aktivitet fra `audit_logs` (`journal`, `check-ins`, `logins`). **Audit standardisering til grafer:** nye `audit_logs` actions via migration `20260422170000_audit_logs_extend_actions_for_activity_dashboards.sql` (`journal.entry_created`, `checkin.submitted`) og logging tilføjet i `POST /api/park/resident-journal`, `POST /api/park/voice-journal` samt `POST /api/park/daily-checkin` (best-effort `create_audit_log`, inkl. org-scope). Derudover logges staff-oprettede journalnotater nu også via `logPortalAudit` i `WriteJournalEntry`, `HurtigJournalModal` og `DagbogEveningTools`, så journal-kurven dækker både Lys- og Care Portal-flow. **Driftsmodul: PIN-nulstilling for ledere:** `/care-portal-residents` er nu en live org-liste (ikke kun demo-grid) med leder-gate (`care_staff.role === 'leder'`). Viser beboerens navn + oprettelsesdato + **Nulstil PIN**. Knap åbner modal med 4-cifret input og gemmer via eksisterende edge function `resident-pin-set` (staff session token). Række markeres med grønt flueben efter succes. **Fleksible roller/rettigheder (fuld prompt-implementering):** Migration `20260421143000_roles_and_permissions.sql` bruger nu **`org_roles.permissions` (text[])** + `is_system_role`, `care_staff.role_id` (legacy `role` gjort nullable), seed/backfill af systemroller (`leder`, `medarbejder`, `gæst`) med default permissions + trigger for nye orgs. Ny `src/lib/permissions.ts`, `src/lib/auth/getStaffPermissions.ts` (React `cache`) og `src/lib/auth/hasPermission.ts`, samt dansk label-map i `src/lib/permissionLabels.ts`. Middleware sætter `x-staff-permissions` header for staff-ruter. Rolle-API: `GET/POST /api/portal/roles`, `PATCH/DELETE /api/portal/roles/[roleId]`. Ny side **`/care-portal-roles`** med liste + editor (opret, redigér navn/permissions, slet custom roller). Invite-flow bruger dynamiske roller fra API og gemmer `role_id` på `care_staff`. Sidebar skjuler nu ledelsesmenuer via permissions (Vagtplan, Importer beboere, Roller). Permission-tjek tilføjet i `import-residents`, `invite-staff`, `journal-polish`, `journal-day-synthesis` + import-side kræver `import_residents`. **Care Portal glemt adgangskode:** Nyt reset-flow med `src/app/care-portal-reset-password/page.tsx` (email + `supabase.auth.resetPasswordForEmail`, redirect til `/care-portal-update-password`) og `src/app/care-portal-update-password/page.tsx` (`supabase.auth.updateUser({ password })` efter token fra mail/hash). Login-sidens “Glemt adgangskode?” linker nu til reset-siden. **Import → PIN-flow:** `ImportWizard` har nu knap **“Sæt PIN for beboere”** efter vellykket CSV-import. Knap åbner liste over de netop importerede (matchet på succes-rækker + `care_residents`-query), med 4-cifret input + **Gem PIN** pr. beboer mod eksisterende edge function `resident-pin-set` (med staff session token). Rækker markeres med grønt flueben ved succes, samlet status viser når alle er sat. **Intern admin (`/budr-admin`):** Ny side til org-oprettelse uden Supabase-dashboard. Formular med `admin_secret` (valideres mod **`BUDR_ADMIN_SECRET`**), org-navn og slug. Server-action opretter `organisations`-række via service role, genererer unik `invite_code`, og viser færdigt invite-link (`<site>/invite/<code>`) klar til leder. Ruten er nu ekstra beskyttet i `middleware.ts` med **HTTP Basic Auth** (`/budr-admin*`): bruger `BUDR_ADMIN_BASIC_USER` (fallback `budr`) + `BUDR_ADMIN_BASIC_PASS` (fallback `BUDR_ADMIN_SECRET`). **Lys:** Genvejen **Aktiviteter** (→ Kalender-fanen) kunne virke død fordi krise-FAB (`absolute`) overlappede nederste højre genvej; FAB er igen **`fixed`** med `right` ud fra **430px**-kolonne + ekstra **paddingBottom** i `LysShell` så indhold ikke ligger under FAB. **Telefon-layout:** `park-hub/layout` + `lys-chat/layout` (`budr-lys-phone-frame` / `budr-lys-phone-column`), overlays, bundmenu; `lys-phone.css`.
