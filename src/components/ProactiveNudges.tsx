...
const fallbackNudges: NudgeData[] = [
  { id: 'n1', message: 'Det er 3 dage siden dit sidste check-in. Vil du tage 2 minutter til at tjekke ind i dag?', pattern: 'Manglende check-in', emoji: '🌅', type: 'checkin' },
  { id: 'n2', message: 'Dine søvnscorer har været lave denne uge. Vil du prøve en kort aftenroutine i aften?', pattern: 'Søvnmønster', emoji: '🌙', type: 'sleep' },
];

export default function ProactiveNudges({ recentMoods = [], lastSleepScore = 3, daysSinceCheckin = 0, weekdayPattern = '' }: ProactiveNudgesProps) {
...
Generer 2-3 personlige, proaktive nudges.`,
  ...
  useEffect(() => {
    ...
    if (daysSinceCheckin > 2 || lastSleepScore < 2 || (recentMoods.length > 0 && recentMoods[recentMoods.length - 1] < 4)) {
      generateNudges();
    }
  ...
  return (
    <div className="space-y-2">
      {visibleNudges.map(nudge => (
        <div key={nudge.id} className="bg-aurora-violet/8 border border-aurora-violet/20 rounded-2xl p-4 animate-slide-up">
