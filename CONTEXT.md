...
**Sidst opdateret (manuelt):** 2026-05-21 — **Critical correctness hotfix:** Beboeradgang er igen PIN/WebAuthn-gated: `/app/<resident_user_id>` og `/api/resident-auth/session?rid=...` må ikke længere minte sessioner fra UUID alene, men sender til `/login/<resident_user_id>`. Server-side beboer-API’er bruger `budr_resident_session` + `resident_sessions.session_token_hash`; `budr_resident_id` er kun client-kompatibilitet/demo-hint. PIN/WebAuthn Edge Functions skriver nu `resident_user_id`, `org_id` og `session_token_hash`, og `/api/resident-session` validerer token før cookies sættes. RLS: ny migration `20260521110500_lock_down_plan_rls_policies.sql` lukker org-agnostiske staff-policies på `daily_plans`, `plan_proposals`, `resident_plan_items`; compat-view migrationen sikrer `lys_checkin.checkin_type` før view-oprettelse.
