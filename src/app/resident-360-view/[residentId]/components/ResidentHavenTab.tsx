...
export default function ResidentHavenTab({ residentId, residentName }: Props) {
  ...
  useEffect(() => {
    ...
    supabase
      .from('garden_plots')
      .select('id, slot_index, plant_type, plant_name, goal_text, growth_stage, total_water, last_watered_at')
      .eq('resident_id', residentId)
      .order('slot_index')
      .then(({ data }) => setPlots((data ?? []) as GardenPlot[]), () => setPlots([]));
  }, [residentId]);
  ...
  if (plots.length === 0) {
    return (
      <div className="rounded-2xl border-2 border-dashed border-gray-200 py-16 text-center">
        ...
        <p className="text-gray-500 font-semibold">{residentName.split(' ')[0]} har ikke plantet noget endnu</p>
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <p className="text-white/70 text-xs font-bold uppercase tracking-wide mb-4">{residentName.split(' ')[0]}s have</p>
        {/* Back row */}
        <div className="flex justify-around items-end mb-2 px-8">
          {slots.slice(0, 3).map((plot, i) => (
            <div key={`b${i}`} className="flex flex-col items-center" style={{ filter: 'brightness(0.85)' }}>
              <div className="w-16 h-16">
                {plot ? <PlantSvg type={plot.plant_type} stage={plot.growth_stage} /> : (
      ...
      <div>
        ...
        <div className="space-y-3">
          {plots.map(plot => {
            ...
            const progress = plot.growth_stage === 4
              ? 100
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 mb-2">
                    ...
                    {plot.last_watered_at && (
                      <> · Sidst {new Date(plot.last_watered_at).toLocaleDateString('da-DK', { day: 'numeric', month: 'short' })}</>
                    )}
                  </p>
                  {plot.goal_text && (
                  ...
                  <div className="h-2 rounded-full bg-gray-100 overflow-hidden">
                    <div
                      ...
                      style={{ width: `${progress}%`, background: `linear-gradient(90deg, ${accent}, ${accent}bb)` }}
                    />
                  </div>
                </div>
              </div>
