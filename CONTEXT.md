...
**Sidst opdateret (manuelt):** 2026-04-25 — **Diagnostik for dashboard-bosted-banner + build-fix til deploy:** `src/lib/supabase/organisation.ts#getOrganisationForStaff()` logger nu server-side diagnose med præfix **`[BUDR-DIAGNOSE]`** for grene: `no_server_client`, `no_user_in_session`, `user_id: <id>, missing_org_id_in_metadata`, `organisations_query_failed`, og `organisations_query_returned_zero_rows`. Ved query-fejl logges også fuld Supabase error payload (`code`, `message`, `details`, `hint`). Formål: isolere hvorfor “mangler tilknytning”-banner kan vises trods sat `auth.users.user_metadata.org_id`. Ingen UI- eller logikændringer i banner-flow. For at afhjælpe build-stop på `/icon.svg` er statisk `src/app/icon.svg` erstattet af metadata-route `src/app/icon.tsx` (PNG via `next/og`), så Next/Netlify bygger uden manglende `route.js` for icon.
