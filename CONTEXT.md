...
**Sidst opdateret (manuelt):** 2026-08-31 — **Live vagtplan + løn:** `/care-portal-vagtplan` og `/loen` mountede `VagtplanDemoClient` / `LoenDemoClient` uden `demoMode={false}` og gemte i `localStorage` (`budr_demo_shifts_v1`) med hashed fiktive kolleger og løn for «Lars N.». Live loader/gemmer nu `care_staff_shifts` (org fra `care_staff`, RLS, kun self-insert/delete). Demo (`demoMode`) uændret localStorage. Migration `20260831110500_care_staff_shifts.sql`.
