...
**Sidst opdateret (manuelt):** 2026-05-20 — **Resident-session sikkerhedshotfix:** `/app/<resident_user_id>` opretter ikke længere session fra URL alene, men kræver gyldig `budr_resident_session` eller sender til `/login/<resident_user_id>` (PIN/WebAuthn). `/api/resident-auth/session` er lukket for anonym session-minting. `getResidentId()` og middleware stoler nu på valideret HttpOnly-session; `budr_resident_id` er kun client-kompatibilitet/demo-cookie. PIN/WebAuthn Edge Functions skriver `resident_sessions` med `resident_user_id`, `org_id` og `session_token_hash`, og `/api/resident-session` validerer token før cookies sættes.
