...
**Sidst opdateret (manuelt):** 2026-07-31 — **Kritisk sikkerhed:** `/app/<resident_user_id>` sender til PIN-login (`/login/<id>?next=/park-hub`) i stedet for session fra UUID. `/api/resident-auth/session` minter ikke sessions fra `rid`/`residentUserId`; `/api/resident-session` validerer token mod `resident_sessions.session_token_hash`. `getResidentId()`/middleware kræver HttpOnly `budr_resident_session` for live-beboere. AI-briefs: migration `20260731110500_ai_briefs_schema_rls.sql` + kun `journal_status='godkendt'` i Anthropic-prompts. `staff-assistant`/`mood-alert` scoped til autentificeret `care_staff.org_id`. Invite-registrering kræver gyldig `invite_code` server-side og tillader ikke selvvalgt `leder`.
