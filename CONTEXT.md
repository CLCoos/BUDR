...
**Sidst opdateret (manuelt):** 2026-04-24 — **Robust org_id-håndtering + setup-flow:** ny central client resolver `src/lib/auth/useAuthenticatedUser.ts` introducerer samlet auth-state (`loading`, `unauthenticated`, `authenticated`, `authenticated-incomplete` med årsag). Bruges nu i `UserMenu`, `PortalSidebar`, `DashboardClient` og `useStaffOrgIsBingbong` for mere stabil identitet/org-kontekst (inkl. mere stabile avatar-initialer og mindre race-condition i org-afhængige UI-stier). Ny menneskelig setup-side: `src/app/care-portal-dashboard/setup/page.tsx` (+ CSS) med rolige beskeder og support-CTA. Middleware (`middleware.ts`) håndterer nu incomplete staff-state (manglende/ugyldig metadata-org, deaktiveret/ukendt org eller manglende/mismatch `care_staff`) og redirecter beskyttede portalruter til `/care-portal-dashboard/setup` (med loop-undtagelse for setup-siden). Intern admin udvidet med værktøj til “Brugere uden fuld tilknytning” via `adminOverview` + `BudrAdminClient` og ny route `POST /budr-admin/attach-user` som opdaterer `auth.user_metadata.org_id` og upserter `care_staff` med valgt org+rolle. Tekniske org_id-fejltekster i dashboard/settings er omskrevet til menneskeligt sprog. **Auth/UI-stabilitet:** `AuthenticatedUserProvider` i `PortalShell` (plus `care-portal-dashboard/setup/layout` og `/design-system` wrapper) så `useAuthenticatedUser` ikke kører i parallel pr. komponent; `useStaffOrgIsBingbong` læser kun `org.slug` fra samme state (ingen ekstra slug-query); sidebar-afdeling hentes via server-`orgId` og viser dropdown først når listen er klar (undgår kort flash af fuld demo-afdelingsliste). Kvalitetscheck: `type-check`, `lint`, `build` grøn.
