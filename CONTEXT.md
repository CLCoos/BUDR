...
**Sidst opdateret (manuelt):** 2026-05-23 — **Kritiske korrekthedsfix:** Resident-session hotfix: `/app/<resident_id>` og `/api/resident-auth/session` mint’er ikke længere sessioner fra UUID alene; live Lys/Park-API’er bruger kun valideret `budr_resident_session` (HttpOnly) via `resident_sessions.session_token_hash`. PIN/WebAuthn edge functions skriver hash-schemaet (`resident_user_id`, `org_id`, `session_token_hash`), `/api/resident-session` validerer token før cookie, middleware accepterer kun live sessioner (legacy `budr_resident_id` kun demo-cookie), og migration `20260522113000_repair_resident_sessions_hashed_schema.sql` reparerer gamle `resident_sessions`-skemaer. Pilot-medicin-widgetten bruger nu Sara-demoens aktuelle beboer-id’er i simuleret Care Portal, så `/care-portal-dashboard` ikke crasher med `Unknown resident res-001` når `NEXT_PUBLIC_CARE_PORTAL_SIMULATED_DATA=true`.
