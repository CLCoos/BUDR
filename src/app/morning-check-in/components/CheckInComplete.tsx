...
const energyForecast: Record<number, { label: string; emoji: string; accent: string; tasks: string }> = {
...
export default function CheckInComplete({ data, onComplete }: Props) {
  ...
  const [aiMessage, setAiMessage] = useState(fallbackMessages[data.mood] || fallbackMessages['rolig']);
  ...
  useEffect(() => {
    ...
    const intentionNote = data.intention ? ` Brugerens intention for i dag: "${data.intention}".` : '';
    setAiLoading(true);
    ...
    fetch('/api/ai/chat-completion', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Du er Lys — en varm, empatisk ledsager i en dansk mental sundhedsapp. Du sender en personlig morgenbesked til brugeren efter deres check-in. Max 2 sætninger, max 30 ord. Vær specifik, varm og nærværende. Afslut med ét relevant emoji.',
          },
          {
            role: 'user',
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pb-12 flex flex-col items-center">
      <div className={`mt-4 mb-6 transition-all duration-700 ${visible ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`}>
        <Lys mood={getLysMood()} size="lg" showMessage userContext={userContext} />
      </div>

      <div className={`text-center mb-6 transition-all duration-700 delay-150 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        <h2 className="font-display text-2xl font-bold text-midnight-50 mb-2">Check-in gennemført! 🌅</h2>
        <p className="text-midnight-400 text-sm">Din dag er klar og tilpasset dit energiniveau</p>
      </div>

      <div className={`w-full mb-4 transition-all duration-700 delay-200 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        <div className="rounded-3xl p-4 flex items-center gap-3 border" style={{ background: `${forecast.accent}12`, borderColor: `${forecast.accent}30` }}>
          <span className="text-4xl select-none flex-shrink-0">{forecast.emoji}</span>
          <div className="min-w-0">
            <p className="font-display font-bold text-lg text-midnight-50 break-words">{forecast.label}</p>
            <p className="text-sm text-midnight-400 mt-0.5 break-words">📋 {forecast.tasks} venter på dig</p>
          </div>
        </div>
      </div>

      {data.intention && (
        <div className={`w-full mb-4 bg-midnight-800/60 rounded-2xl border border-emerald-500/20 p-4 transition-all duration-700 delay-250 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <p className="text-xs text-emerald-400 font-semibold mb-1.5">🌱 Din intention i dag:</p>
          <p className="text-sm text-midnight-100 font-medium italic">&ldquo;{data.intention}&rdquo;</p>
        </div>
      )}

      <div className={`w-full mb-6 bg-midnight-800/60 rounded-3xl border border-aurora-violet/20 p-5 transition-all duration-700 delay-300 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
