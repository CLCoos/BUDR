...
- **Marketing / budrcare.dk forside:** `src/components/marketing/CareEntrySplit.tsx` — split-skærm entry med mørk venstre panel (borgere/pårørende → `/institutioner`) og lys højre panel (Care Portal → `/care-portal-login`). SVG grid-baggrund, opacity hover-overlay (ingen flex-reflow), centreret divider-orb, feature hints, splash-animation. `src/app/budr-landing.css` indeholder alle `.care-entry-*`, `.budr-panel-*`, `.budr-divider*`, `.budr-cta-dark/green`, `.budr-feature-hint*`, `.budr-fade-in` klasser. `src/components/marketing/HomeLanding.tsx` + `src/app/budr-landing.css` — dansk copy, **hero**, **to-trins overskrift**; `src/app/page.tsx` Open Graph. **Institutions-CMS:** hero/CTA/pilot-link på `/institutioner` kan redigeres/publiceres uden kode via `care-portal-dashboard/settings`.
