...
function seedLocalStorage() {
  ...
  const journal = [
    { id:'demo-j-0', date:todayStr(0), mode:'write', text:'Stod op til en rigtig god morgen. Solskinnet var allerede fremme, og jeg nåede min morgenmeditation inden morgenmaden. Gik en tur i haven — solsikken er ved at slå ud. Det er mærkeligt, at noget så simpelt som at se en blomst vokse kan give så meget ro. Tager den kreative tid seriøst i dag — har sat mig for at male det grønne hjørne af haven.', mood:8, feelings:['Glad','Rolig','Håbefuld'], privacy:'shared', prompt:'Hvad gav dig energi i dag?' },
    { id:'demo-j-1', date:todayStr(1), mode:'write', text:'God dag i går. Fuldførte alle aktiviteter — endda aftensmaden med gruppen, som jeg ellers plejer at springe over. Lys spurgte om det, og det fik mig til at reflektere over, hvad der er anderledes nu. Måske er det rutinen. Måske er det solsikken.', mood:7, feelings:['Rolig','Taknemmelig'], privacy:'private', prompt:'Hvad gjorde du anderledes i dag?' },
  ...
  const planItems = [
    ...
    { id:'demo-pi-5', resident_id:DEMO_RESIDENT_ID, title:'Kreativ tid (maleri)',  category:'Kreativitet',emoji:'🎨', time_of_day:'14:00', recurrence:'daily',    recurrence_days:null,      notify:true,  notify_minutes_before:15, created_by:'resident', staff_suggestion:false, approved_by_resident:true, active_from:todayStr(7),  created_at:daysAgo(7)  },
    ...
    { id:'demo-pi-7', resident_id:DEMO_RESIDENT_ID, title:'Aftenmedicin',          category:'Medicin',    emoji:'💊', time_of_day:'20:00', recurrence:'daily',    recurrence_days:null,      notify:true,  notify_minutes_before:10, created_by:'staff',    staff_suggestion:true,  approved_by_resident:true, active_from:todayStr(13), created_at:daysAgo(13) },
  ];
  ...
  const completions = [
    ...
    { id:'demo-pc-3', resident_id:DEMO_RESIDENT_ID, plan_item_id:'demo-pi-3', completion_date:todayStr(0), completed_at:daysAgo(0, 10, 20) },
    { id:'demo-pc-4', resident_id:DEMO_RESIDENT_ID, plan_item_id:'demo-pi-4', completion_date:todayStr(0), completed_at:daysAgo(0, 12, 10) },
    { id:'demo-pc-5', resident_id:DEMO_RESIDENT_ID, plan_item_id:'demo-pi-5', completion_date:todayStr(0), completed_at:daysAgo(0, 14, 35) },
  ];
  ...
  localStorage.setItem(`budr_dag_completed_${todayStr(1)}`, JSON.stringify(['demo-pi-1','demo-pi-2','demo-pi-3','demo-pi-4','demo-pi-5','demo-pi-6']));

  // 2 days ago — 4 done
  localStorage.setItem(`budr_dag_completed_${todayStr(2)}`, JSON.stringify(['demo-pi-1','demo-pi-2','demo-pi-3','demo-pi-4']));

  // Today's check-in (Humørtjek on hjem)
  localStorage.setItem('budr_last_checkin', JSON.stringify({ level: 4, label: 'Godt', ts: Date.now() - 25 * 60 * 1000 }));

  // Kriseplan udfyldt med 3 af 4 sektioner
  localStorage.setItem('budr_kriseplan_v1', JSON.stringify({
    version: 1,
    updated_at: daysAgo(2),
    sections: {
      feeling_ok: 'Jeg sover godt, spiser morgenmad og har lyst til at male eller gå en tur. Jeg er nærværende og taler gerne med andre.',
      early_signs: 'Jeg trækker mig fra fællesskabet, sover dårligt og spiser uregelmæssigt. Jeg bliver mere irritabel og har svært ved at koncentrere mig.',
      what_helps: 'At gå en tur i naturen, male, lytte til musik eller tale med en af personalet jeg har det godt med. Ro og tid til mig selv hjælper.',
    },
