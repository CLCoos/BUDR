...
export async function GET(): Promise<NextResponse> {
  ...
  if (!orgId) {
    return NextResponse.json({ error: 'Manglende org_id på bruger' }, { status: 403 });
  }
  ...
  const draft = sanitizeInstitutionerHeroCopy(data?.draft ?? DEFAULT_INSTITUTIONER_HERO_COPY);
  const published = data?.published ? sanitizeInstitutionerHeroCopy(data.published) : null;
  const revisions = sanitizeRevisions(data?.revisions).map((r) => ({
    id: r.id,
    created_at: r.created_at,
    actor_id: r.actor_id,
    action: r.action,
    snapshot: sanitizeInstitutionerHeroCopy(r.snapshot),
  }));
...
export async function PUT(req: Request): Promise<NextResponse> {
  ...
  try {
    body = (await req.json()) as MarketingBody;
  } catch {
  ...
  const fallbackDraft = sanitizeInstitutionerHeroCopy(
    row?.draft ?? DEFAULT_INSTITUTIONER_HERO_COPY
  );
  const rollbackTarget = rollbackRevisionId
    ? revisions.find((r) => r.id === rollbackRevisionId)
    : null;
  const draft = rollbackTarget
    ? sanitizeInstitutionerHeroCopy(rollbackTarget.snapshot)
    : sanitizeInstitutionerHeroCopy(body.draft ?? fallbackDraft);
  ...
  return NextResponse.json({
    ok: true,
    draft,
    published: rollbackTarget || publish ? draft : null,
    rollback_applied: !!rollbackTarget,
  });
}
