...
**Sidst opdateret (manuelt):** 2026-08-12 — **Critical: resident auth-tabeller + journal_status + crisis_plans DELETE:** Migration `20260812110500_resident_auth_tables_journal_status_crisis_delete.sql` opretter manglende `resident_pins` / `resident_webauthn_credentials` (service-role-only RLS; Edge PIN/WebAuthn brugte tabeller der aldrig fandtes i aktiv migrationskæde), tilføjer `journal_entries.journal_status` + `approved_at`/`approved_by` (kladde/godkendt — kræves af Lys journal + besked-til-personale), og fjerner staff hard-DELETE på `crisis_plans`. Post-merge: `supabase db push --linked --include-all`.
...
- **Aftenopsamling / journal-schema:** `journal_status` / `approved_at` / `approved_by` bringes ind via aktiv migration `20260812110500_resident_auth_tables_journal_status_crisis_delete.sql` (archive `20260404120000_*` er ikke på kæden). UI-fallback (`journalEntriesQueryCompat.ts`, dagbog/HurtigJournal) forbliver for ældre miljøer indtil `db push`. `show_in_diary` findes allerede i baseline.
