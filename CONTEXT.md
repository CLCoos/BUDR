...
**Sidst opdateret (manuelt):** 2026-08-29 — **Live dashboard-opgaver:** `OpgaveWidget` (`demoMode=false` på `/care-portal-dashboard` og `/care-portal-planner`) viser ikke længere fiktive Solhaven-opgaver (Finn L./kriseplan) og gemmer «Gem opgave» + status i `care_staff_tasks` (org fra `care_staff`, RLS + `care_staff_can_access_resident`). Demo (`demoMode`) og pilot-sim er uændret in-memory Sara-univers. Migration `20260829110500_care_staff_tasks.sql`.
