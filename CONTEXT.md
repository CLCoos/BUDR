...
**Sidst opdateret (manuelt):** 2026-08-15 — **Lys krisekort (cookie-session):** `LysKrisekort` læste `crisis_plans`, `facility_contacts` og `on_call_staff` via browser-klienten + `supabase.auth.getUser()`. Live Lys har kun `budr_resident_id`-cookie (ingen JWT), og RLS kræver `auth.uid()` → tom kriseplan og ingen bostedstelefoner i en krise. Ny `GET /api/lys/crisis-support` (cookie + service role, org fra `care_residents`, vagt-skift i Europe/Copenhagen). Tests: `src/lib/lys/crisisSupport.test.ts`.
...
- **Lys krisekort:** `GET /api/lys/crisis-support` (cookie + service role) leverer kriseplan, facility-kontakter og aktuel vagthavende til `LysKrisekort`. Alert-knappen bruger fortsat `POST /api/lys/crisis-alert`.
