...
**Sidst opdateret (manuelt):** 2026-08-25 — **Lys logout:** `DELETE /api/resident-session` tilbagekalder `resident_sessions` (hash-match) og rydder både `budr_resident_session` (HttpOnly) og `budr_resident_id` med samme cookie-attributter som ved sæt. Lys «Log ud» kalder ruten i stedet for kun at nulle den læsbare cookie. Delt `clearResidentAuthCookies` bruges også i middleware.
...
- **Middleware:** `middleware.ts` — park-ruter vs. `care_residents`; demo-portal bag env-flag; staff via `care_staff` (inkl. **`/design-system`**); se **Sidst opdateret** ovenfor. Lys-logout rydder session-cookies via `DELETE /api/resident-session` (HttpOnly kan ikke slettes i browser-JS).
