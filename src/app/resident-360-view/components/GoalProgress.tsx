...
const goals: Goal[] = [
  {
    ...
    steps: [
      { id: 'gps-001-1', text: 'Gå til postkassen en gang om dagen', completed: true, completedAt: '24/03/2026' },
      { id: 'gps-001-2', text: 'Tag en 5 minutters tur rundt om bygningen', completed: true, completedAt: '25/03/2026' },
    ...
    steps: [
      { id: 'gps-002-1', text: 'Sluk skærm 30 min før sengetid', completed: true, completedAt: '23/03/2026' },
  ...
  },
  {
    ...
    steps: [
      { id: 'gps-003-1', text: 'Sidde med ved fællesspisning én gang', completed: true, completedAt: '22/03/2026' },
            ...
            <div key={goal.id}>
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <span className="text-xs font-bold tabular-nums flex-shrink-0" style={{ color: '#1D9E75' }}>{pct}%</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="text-xs text-gray-400">{done}/{total} trin · {goal.createdBy}</div>
                        ...
                        return (
                          <div key={step.id} className="flex items-start gap-3 relative">
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 z-10 ${
                              step.completed ? 'bg-[#1D9E75]' : isLocked ? 'bg-gray-100' : 'bg-white border-2 border-[#1D9E75]/40'
                            }`}>
                              {step.completed
                                ? <CheckCircle2 size={12} className="text-white" />
                                : <Circle size={12} className={isLocked ? 'text-gray-300' : 'text-[#1D9E75]/40'} />
                              }
                            </div>
                            <div className={`flex-1 pt-0.5 ${isLocked ? 'opacity-40' : ''}`}>
                              <div className={`text-xs ${step.completed ? 'line-through text-gray-400' : 'text-gray-700'}`}>
                                {step.text}
                              </div>
                              {step.completedAt && (
                                <div className="text-xs text-[#1D9E75] mt-0.5">✓ {step.completedAt}</div>
                              )}
                            </div>
                          </div>
                        );
