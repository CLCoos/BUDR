...
export default function OverrapportPanel({ open, onClose }: Props) {
  ...
  useEffect(() => {
    if (!open) { setEditMode(false); setSaved(false); return; }
    void fetchAll();
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [resResult, journalResult, checkinResult, planResult] = await Promise.all([
        supabase.from('care_residents').select('user_id', { count: 'exact', head: true }),
        supabase
      ...
      const journalResidentIds = [...new Set((journalResult.data ?? []).map((j: { resident_id: string }) => j.resident_id))];
      const checkinResidentIds = [...new Set((checkinResult.data ?? []).map((c: { resident_id: string }) => c.resident_id))];
      ...
      if (allResidentIds.length > 0) {
        ...
        nameMap = Object.fromEntries((residents ?? []).map((r: { user_id: string; display_name: string }) => [r.user_id, r.display_name]));
      }

      if (!mountedRef.current) return;

      setJournals(
        (journalResult.data ?? []).map((j: { id: string; entry_text: string; category: string; staff_name: string; created_at: string; resident_id: string }) => ({
          ...j,
      ...
      setCheckins(
        (checkinResult.data ?? []).map((c: { resident_id: string; mood_score: number; traffic_light: string; note: string | null; created_at: string }) => ({
          ...c,
      ...
      const planResidentIds = [...new Set((planResult.data ?? []).map((p: { resident_id: string }) => p.resident_id))];
      let planNameMap: Record<string, string> = { ...nameMap };
      ...
      if (missingIds.length > 0) {
        ...
        (planResidents ?? []).forEach((r: { user_id: string; display_name: string }) => { planNameMap[r.user_id] = r.display_name; });
      }

      for (const plan of (planResult.data ?? []) as { resident_id: string; date: string; plan_items: PlanItem[] | null }[]) {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (mountedRef.current) { setSaved(true); setEditMode(false); }
    } catch { /* ignore */ }
    finally { if (mountedRef.current) setSaving(false); }
  };

  const attentionCheckins = checkins.filter(c => c.traffic_light === 'rød' || c.traffic_light === 'gul');
    ...
    <>
      ...
      >
          ...
          {loading ? (
            <div className="flex flex-col items-center py-16 gap-3 text-gray-400">
              <div className="flex gap-1.5">
                {[0, 150, 300].map(d => (
                  <span key={d} className="w-2 h-2 bg-[#1D9E75] rounded-full animate-bounce" style={{ animationDelay: `${d}ms` }} />
                ))}
              </div>
            ...
            <div className="px-6 py-5 space-y-6">
              ...
              <section>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Vagtoverblik</h3>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { label: 'Beboere tilstede', value: totalResidents, color: '#1D9E75', bg: '#E1F5EE' },
                    { label: 'Check-ins i dag', value: checkins.length, color: '#185FA5', bg: '#E6F1FB' },
                    { label: 'Åbne advarsler', value: attentionCheckins.length, color: '#E24B4A', bg: '#FEF2F2' },
                  ].map(stat => (
              ...
              <section>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Journalnotater i dag</h3>
                  <span className="ml-auto text-xs text-gray-400">{journals.length} noter</span>
                </div>
                {journals.length === 0 ? (
                  <p className="text-sm text-gray-400 text-center py-4">Ingen journalnotater i dag</p>
                ) : (
                          ...
                          return (
                            <div key={e.id} className="px-3 py-2.5">
                              <div className="flex items-center gap-2 mb-1.5">
                                <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: cat.bg, color: cat.color }}>
                                  {cat.label}
                                </span>
                                <span className="text-[10px] text-gray-400">{fmt(e.created_at)} · {e.staff_name}</span>
                              </div>
                              <p className="text-xs text-gray-700 leading-relaxed">{e.entry_text}</p>
                            </div>
                          );
                        })}
                      </div>
              ...
              <section>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Stemningsoverblik</h3>
                </div>
                {attentionCheckins.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {attentionCheckins.map(c => {
                      ...
                      return (
                        <div key={c.resident_id} className="flex items-center gap-3 rounded-xl border border-gray-100 px-3 py-2.5">
                          <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: tc.dot }} />
                          <div className="flex-1 min-w-0">
                            <span className="text-sm font-medium text-gray-800">{c.resident_name}</span>
                            {c.note && <p className="text-xs text-gray-500 truncate">{c.note}</p>}
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className="text-sm font-bold tabular-nums" style={{ color: tc.text }}>
                              {c.mood_score}<span className="text-xs font-normal text-gray-400">/10</span>
                            </span>
                    ...
                    {checkins.filter(c => c.traffic_light === 'grøn').map(c => (
                      <div key={c.resident_id} className="flex items-center gap-3 rounded-xl border border-gray-100 px-3 py-2.5 opacity-60">
                        <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: '#1D9E75' }} />
              ...
              {openTasks.length > 0 && (
                <section>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Åbne opgaver</h3>
                    ...
                    {openTasks.map((t, i) => (
                      <div key={i} className="flex items-center gap-3 rounded-lg border border-gray-100 px-3 py-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <span className="text-[10px] text-gray-400 ml-1.5">· {t.resident_name}</span>
                        </div>
                        {t.time && <span className="text-[10px] text-gray-400 flex-shrink-0">{t.time}</span>}
                      </div>
                    ))}
                  </div>
                </section>
              ...
              {editMode && (
                <section>
                  <label className="block text-xs font-semibold text-gray-600 mb-2">Rediger rapport</label>
                  <textarea
                    value={editText}
        ...
        <div className="flex gap-2 px-6 py-4 border-t border-gray-100 shrink-0">
          <button
            type="button"
            onClick={() => { setEditMode(!editMode); if (!editMode) setEditText(''); }}
