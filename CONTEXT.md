...
**Sidst opdateret (manuelt):** 2026-04-25 — **Dashboard-banner root-cause + kompatibilitetsfix:** Prod-logs (`[BUDR-DIAGNOSE]`) viste, at banneret ikke skyldtes manglende `org_id`, men SQL-fejl `42703` fordi `organisations.resident_name_display_mode` manglede i miljøet. `src/lib/supabase/organisation.ts#getOrganisationForStaff()` har nu fallback-query: prøv først med `resident_name_display_mode`; ved `42703` retries samme org-opslag uden kolonnen (default mode fortsat `first_name_initial`). Dermed undgås falsk “mangler tilknytning”-banner i miljøer uden den migration. Diagnostiske logs er bevaret. Tidligere samme dag: build-fix ved at erstatte `src/app/icon.svg` med metadata-route `src/app/icon.tsx` for stabil Netlify/Next-build.
