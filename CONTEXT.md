...
**Sidst opdateret (manuelt):** 2026-04-24 — **Sidebar-gruppering (Care Portal):** `PortalSidebar` er omstruktureret fra flad nav-liste til grupper (`Overblik`, `Dagligt arbejde`, `Planlægning`, `Værktøjer`) med uppercase gruppe-labels og subtil divider før bundsektion. Aktiv-state er uændret. I kollapset sidebar skjules gruppe-labels, links viser kun ikoner og tooltip via `title`, og divider rendres som lille prik. Bunden har fortsat `Indstillinger`-ikon, tema-toggle (kun udvidet) og log ud. Tidligere ændring fra samme dag: **Header-refaktor (Care Portal)** med `TopHeader` (logo + søgning + brugermenu), `UserMenu`, afdelingsvælger flyttet til org-blok, og `/design-system` sektioner for `TopHeader`, `UserMenu` samt nu også `Layout / Sidebar` states. Kvalitetscheck: `type-check`, `lint`, `build` grøn.
