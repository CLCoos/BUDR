...
export default function LysJournalTab({ tokens, accent }: Props) {
  ...
  const saveLetters = (updated: SelfLetter[]) => {
    try {
      localStorage.setItem(LOCAL_KEYS.selfLetters, JSON.stringify(updated));
    } catch {
      /* ignore */
    }
    setLetters(updated);
  ...
  useEffect(() => {
    ...
    dataService
      .getPlanItems(storageMode, residentId)
      .then((items) => {
  ...
  const fetchLysMessage = async (
    ...
    try {
      ...
      const historyLines = recentEntries.slice(0, 3).map((e) => {
        const d = new Date(e.date).toLocaleDateString('da-DK', {
          weekday: 'short',
          day: 'numeric',
        });
          ...
          messages: [
            {
              role: 'system',
              content:
                'Du er Lys — en varm, empatisk ledsager i en dansk mental sundhedsapp. Skriv én kort, oprigtig og personlig kommentar (maks 2 sætninger, maks 35 ord) til hvad borgeren netop har skrevet. Hvis du ser et positivt mønster på tværs af de seneste indlæg, fremhæv det varmt og anerkendende. Slut med ét roligt emoji.',
            },
            {
              role: 'user',
      ...
      if (mountedRef.current) setLysMsgLoading(false);
    }
  };
  ...
  const toggleFeeling = (word: string) => {
    setFeelings((prev) => (prev.includes(word) ? prev.filter((f) => f !== word) : [...prev, word]));
  };
      ...
      <div className="px-5 pt-5 pb-3">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm capitalize mt-0.5" style={{ color: tokens.textMuted }}>
              {today}
            </p>
          </div>
          {streak >= 2 && (
        ...
        {openLetter && (
          ...
          >
            ...
            <p className="text-xs mb-3" style={{ color: tokens.textMuted }}>
              Skrevet{' '}
              {new Date(openLetter.written_at).toLocaleDateString('da-DK', {
                day: 'numeric',
                month: 'long',
                year: 'numeric',
              })}
            ...
            <>
                  ...
                  {MOOD_OPTIONS.map((opt) => (
                    ...
                    >
                      ...
                      <span
                        ...
                        style={{ color: mood === opt.value ? accent : tokens.textMuted }}
                      >
                        {opt.label}
                      </span>
                    </button>
              ...
              >
                <p
                  ...
                  style={{ color: tokens.textMuted }}
                >
                  Hvad mærker du? <span className="font-normal normal-case">vælg gerne flere</span>
                </p>
                <div className="flex flex-wrap gap-2">
                  {FEELINGS.map(({ word, valence }) => {
                    const selected = feelings.includes(word);
                    const chipColor =
                      valence === 1 ? '#22c55e' : valence === -1 ? '#f87171' : accent;
                    return (
                      <button
                        key={word}
                        type="button"
              ...
              {entries.length > 0 && (
                <div>
                  <p
                    className="text-xs font-bold uppercase tracking-wide mb-3 mt-2"
                    style={{ color: tokens.textMuted }}
                  >
                    Tidligere indlæg
                  </p>
                        ...
                        >
                          ...
                          {expanded && (
                            ...
                            >
                              ...
                              {entry.prompt && (
                                <p
                                  className="text-xs italic mt-3 mb-2"
                                  style={{ color: tokens.textMuted }}
                                >
            ...
            {pending.map((l) => (
              ...
              >
                <p className="text-xs" style={{ color: tokens.textMuted }}>
                  🔒 Åbnes{' '}
                  {new Date(l.deliver_at).toLocaleDateString('da-DK', {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric',
                  })}
            ...
            {delivered.map((l) => (
              ...
              >
                <p className="text-xs mb-2" style={{ color: tokens.textMuted }}>
                  Skrevet{' '}
                  {new Date(l.written_at).toLocaleDateString('da-DK', {
                    day: 'numeric',
                    month: 'short',
                    year: 'numeric',
                  })}
