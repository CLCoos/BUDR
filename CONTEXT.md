...
**Sidst opdateret (manuelt):** 2026-08-13 — **Lys-samtaler persist (cookie-beboere):** `/lys-chat` skrev direkte til `lys_conversations` med browser-klienten. RLS kræver `auth.uid() = resident_id`, men live beboere har kun `budr_resident_id`-cookie (ingen JWT) → inserts fejlede lydløst, 360° «Lys-samtaler» var tom, og safety-events fik ikke et gyldigt `conversation_id` (FK-fejl kunne droppe hele eventet). Ny `GET`/`POST` `/api/lys/conversations` (cookie + service role). `insertSafetyEvent` kæder kun `conversation_id` når rækken tilhører samme beboer.
