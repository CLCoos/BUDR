...
**Sidst opdateret (manuelt):** 2026-08-28 — **Live dashboard-kalender:** `KalenderWidget` (`variant=live` på `/care-portal-dashboard` og `/care-portal-planner`) viser ikke længere Sara-univers-mockaftaler og gemmer «Gem aftale» i `care_planner_entries` (org fra `care_staff`). Demo (`variant=demo`) er uændret mock. Migration `20260828110500_care_planner_entries_staff_fields.sql` tilføjer `location` / `responsible` / `house`.
