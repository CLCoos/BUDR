...
**Sidst opdateret (manuelt):** 2026-04-29 — **Forside finpuds (DayInLifeDemo):** layout og typografisk rytme i `src/components/marketing/DayInLifeDemo.module.css` er strammet for bedre visuel sammenhæng (centreret max-bredde, mere konsekvent spacing, bedre alignment mellem "Før"/"Med BUDR"-kort, tydeligere quote/outro-separation). Verificeret med lokal `npm run build`.
