...
**Sidst opdateret (manuelt):** 2026-06-25 — **AI-briefs (kritisk schema/RLS + kladdegrænse):** `ai_briefs` er nu versioneret i migration `20260625110500_ai_briefs_schema_rls.sql` med staff-scoped SELECT/INSERT. `generateBriefForResident` bruger kun `journal_status='godkendt'` som journalinput til Anthropic (legacy fallback kun når kolonnen mangler). Regressioner: `src/lib/aiBriefMigration.test.ts`, `src/lib/ai/generateBrief.test.ts`.
