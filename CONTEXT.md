...
**Sidst opdateret (manuelt):** 2026-04-21 — **Lys:** Genvejen **Aktiviteter** (→ Kalender-fanen) kunne virke død fordi krise-FAB (`absolute`) overlappede nederste højre genvej; FAB er igen **`fixed`** med `right` ud fra **430px**-kolonne + ekstra **paddingBottom** i `LysShell` så indhold ikke ligger under FAB. **Telefon-layout:** `park-hub/layout` + `lys-chat/layout` (`budr-lys-phone-frame` / `budr-lys-phone-column`), overlays, bundmenu; `lys-phone.css`.
