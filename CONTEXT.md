...
**Sidst opdateret (manuelt):** 2026-04-29 — **Deploy-fix (Prettier i portal-indstillinger):** `src/app/care-portal-dashboard/settings/components/SettingsClient.tsx` er formatteret med Prettier for at fjerne skjulte/ugyldige whitespace-token-fejl, som stoppede `next build` i Netlify-lint-trinnet. Verificeret lokalt med `npx prettier --check` + `next lint --file`.
