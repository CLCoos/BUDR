...
- **Marketing / budrcare.dk:** `src/components/marketing/HomeLanding.tsx` + `src/app/budr-landing.css` — dansk copy (bl.a. CTA og sammenligning), **hero** med let baggrunds-indtoning og **to-trins overskrift** (borger → team); `src/app/page.tsx` med strammere `metadata` / Open Graph. **Institutions-CMS:** hero/CTA/pilot-link på `/institutioner` kan redigeres/publiceres uden kode via `care-portal-dashboard/settings`.
