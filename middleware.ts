...
export async function middleware(req: NextRequest) {
  ...
  if (isResidentRoute(pathname)) {
    ...
    // HttpOnly session (PIN/WebAuthn verified) is required for live residents.
    if (sessionToken) {
    ...
    // A readable UUID cookie is only a compatibility hint; never authorize live access with it.
    return failAndMaybeBlock();
  }
