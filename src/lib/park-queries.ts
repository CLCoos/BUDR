...
export async function upsertResourceProfile(
  residentId: string,
  petals: Partial<
    Omit<ResourceProfile, 'id' | 'resident_id' | 'version' | 'created_at' | 'updated_at'>
  >
) {
  const supabase = getSupabase();
...
export async function completeStep(stepId: string, residentNote?: string): Promise<GoalStep> {
  const supabase = getSupabase();
  const { data, error } = await supabase
...
export function subscribeToAlerts(callback: (alert: unknown) => void) {
  const supabase = getSupabase();
  return supabase
    .channel('park_alerts')
    .on(
