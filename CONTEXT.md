...
**Sidst opdateret (manuelt):** 2026-06-28 — **Kritisk sikkerhed/AI-brief hotfix:** Live beboerlogin kræver igen PIN/WebAuthn-token: `/app/[resident_id]` går til `/login/[resident_id]`, `/api/resident-auth/session` udsteder ikke længere session fra UUID alene, `/api/resident-session`, middleware og `getResidentId()` validerer `budr_resident_session` før server-side adgang (læsbart `budr_resident_id` er kun client-hint/demo). Supabase Edge PIN/WebAuthn/session-funktioner er alignet til `resident_sessions.session_token_hash` / `resident_user_id`. AI-briefs har ny migration `20260627110500_ai_briefs_schema_rls.sql` (`ai_briefs` + org-scoped RLS), og `generateBriefForResident` sender kun `journal_status='godkendt'` journalnotater til Anthropic (legacy fallback kun hvis kolonnen mangler). `staff-assistant` og `mood-alert` bruger nu autentificeret `care_staff.org_id` før service-role læs/skriv, så prompt-kontekst og notifikationer ikke kan krydse organisationer. Regressionstests: `residentAuthSecurityRegression.test.ts`, `aiBriefMigration.test.ts`, `generateBrief.test.ts`.
