...
**Sidst opdateret (manuelt):** 2026-08-27 — **Live indsatsdokumentation persist:** Magtanvendelse/indsatsnotater (§136/§141) på live dashboard og `/care-portal-indsatsdok` gemmes i `care_indsats_records` (org-RLS via `care_staff.org_id`, ingen DELETE). UI viser ikke længere «Gemt ✓» uden database-skrivning. Demo (`/care-portal-demo/indsatsdok` + demo-dashboard) bliver i localStorage. Migration `20260827110500_care_indsats_records.sql` — kør `supabase db push --linked --include-all` efter merge.
...
- **Care Portal — indsatsdokumentation:** Live `/care-portal-indsatsdok` og dashboard-modal skriver `care_indsats_records` (org-RLS). Demo-rute/dashboard bruger localStorage. Migration `20260827110500_care_indsats_records.sql`.
