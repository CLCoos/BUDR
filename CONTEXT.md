...
**Sidst opdateret (manuelt):** 2026-08-21 — **Lys PARK-udkast (voice-journal):** `POST /api/lys/voice-journal` gemmer nu journal som **kladde** (ikke default `godkendt`) og returnerer `content` som alias af `journal_note`, så «Lav PARK-udkast med AI» viser udkastet i stedet for et tomt felt. `LysJournalTab` læser `journal_note ?? content`. Hjælpere + tests: `voiceJournalDraft.ts`.
