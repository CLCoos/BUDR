...
**Sidst opdateret (manuelt):** 2026-09-02 — **360° Mål-widget:** Live `GoalProgress` læser `lys_next_steps` (status `aktiv`) i stedet for droppede `park_goals` / `park_goal_steps`. Recovery-fanen var allerede på `getActiveNextSteps`; overblik viste ellers «Ingen aktive mål». Mapper: `src/lib/goalProgressFromNextSteps.ts`.
