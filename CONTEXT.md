...
**Sidst opdateret (manuelt):** 2026-07-30 — **Resident auth hardening + AI-briefs:** `/app/<resident_user_id>` sender nu til PIN-login (`/login/<id>?next=/park-hub`) i stedet for at oprette session fra UUID. `/api/resident-auth/session` kan ikke længere minte sessions fra `rid`/`residentUserId`; `/api/resident-session` validerer token mod `resident_sessions.session_token_hash` før cookie sættes. `getResidentId()` og middleware kræver HttpOnly `budr_resident_session` for live-beboere (legacy `budr_resident_id` kun demo-hint). Supabase Edge PIN/WebAuthn/session-funktioner bruger `resident_user_id` + `session_token_hash`. Accidental duplicate Lys API handlers (`route 2.ts`, `status 2`, `[id] 2`) er fjernet. AI-briefs har nu migration `20260730110500_ai_briefs_schema_rls.sql` med tenant-scoped RLS/service-role cron-adgang, og `generateBriefForResident` sender kun `journal_status='godkendt'` journalnotater til Anthropic (legacy fallback hvis kolonnen mangler).
